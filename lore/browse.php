...
(function() {
    ...
    tabs.forEach(function(tab) {
        tab.addEventListener('click', function() {
            ...
            xhr.open('GET', '<?php echo BASE_URL; ?>lore/ajax_browse.php?type=' + type);
            xhr.onload = function() {
                if (xhr.status === 200) {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            searchTimer = setTimeout(function() {
                ...
                var xhr = new XMLHttpRequest();
                xhr.open('GET', '<?php echo BASE_URL; ?>lore/ajax_browse.php?type=' + type + '&q=' + encodeURIComponent(q));
                xhr.onload = function() {
                    if (xhr.status === 200) {
                        results.innerHTML = xhr.responseText;
                        attachCardToggle();
